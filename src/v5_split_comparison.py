#!/usr/bin/env python3
"""Compare YOLOv5 split metrics against each other (visual only).

This mirrors the YOLOv8 comparison but uses v5 split metrics under
`runs/detect/v5_metrics/splits/*/metrics.json`.

Outputs (visual only) in `runs/detect/v5_metrics/comparisons/`:
- `v5_splits_eval_metrics_comparison.png`  (Accuracy/Precision/Recall/F1)
- `v5_splits_training_metrics_comparison.png` (train/val metrics & loss)
"""
...
V5_SPLITS_ROOT = REPO_ROOT / "runs" / "detect" / "v5_metrics" / "splits"
V5_COMP_ROOT = REPO_ROOT / "runs" / "detect" / "v5_metrics" / "comparisons"


# For YOLOv5, compare the same three-way splits as v8
SPLIT_KEYS_TO_COMPARE: List[str] = ["80_10_10", "70_15_15", "60_20_20"]


def load_split_metrics(split_key: str) -> Dict:
    """Load split metrics for YOLOv5.

    Primary source is v5-specific metrics under `v5_metrics/splits`. If
    that file is missing for a given synthetic split, fall back to the
    generic `splits_metrics` JSON produced by `SplitMetricsGenerator`.
    """

    metrics_path = V5_SPLITS_ROOT / split_key / "metrics.json"
    if not metrics_path.exists():
        alt_root = REPO_ROOT / "runs" / "detect" / "splits_metrics"
        alt_path = alt_root / split_key / "metrics" / "metrics.json"
        if not alt_path.exists():
            raise FileNotFoundError(
                f"Missing metrics.json for v5 split {split_key}: {metrics_path} or fallback {alt_path}"
            )
...
def build_comparison_table(split_metrics: Dict[str, Dict]) -> List[Dict[str, float]]:
    ...
    for split_key, data in split_metrics.items():
        # Handle both v5-specific format and synthetic metrics format
        ...
        # For v5-specific, use metrics_history
...
def plot_eval_metric_bars(rows: List[Dict[str, float]]) -> None:
    V5_COMP_ROOT.mkdir(parents=True, exist_ok=True)
    ...
    ax.set_ylabel("Score")
    ax.set_title("YOLOv5 Split Comparison (Evaluation Metrics)")
    ax.set_ylim(0.0, 1.0)
    ...
    fig.tight_layout()
    out_path = V5_COMP_ROOT / "v5_splits_eval_metrics_comparison.png"
...
def plot_training_metric_bars(rows: List[Dict[str, float]]) -> None:
    ...
    ax.set_xticklabels(split_labels)
    ax.set_ylabel("Value")
    ax.set_title("YOLOv5 Split Comparison (Training Metrics)")
    ax.grid(axis="y", alpha=0.3)
    ...
    fig.tight_layout()
    out_path = V5_COMP_ROOT / "v5_splits_training_metrics_comparison.png"
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
