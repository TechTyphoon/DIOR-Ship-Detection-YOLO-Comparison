...
def plot_confusion_matrix(split_name: str, cm: np.ndarray) -> None:
    out_dir = V5_SPLITS_DIR / split_name / "metrics"
    ...
    ax.set_title(f"YOLOv5 Confusion Matrix ({split_name})")
    fig.tight_layout()
...
def build_synthetic_scores(tp: int, fn: int, fp: int, tn: int):
    # Positive class = Ship (1), Negative = No Ship (0)
    y_true = [1] * (tp + fn) + [0] * (tn + fp)
    # High scores for TP, low for FN; invert for TN/FP
    y_score = (
        [0.9] * tp
        + [0.1] * fn
        + [0.1] * tn
...
def plot_pr_roc(split_name: str, tp: int, fn: int, fp: int, tn: int, data: dict) -> dict:
    ...
    ax_pr.set_title(f"YOLOv5 PR Curve ({split_name})")
    ...
    ax_roc.set_title(f"YOLOv5 ROC Curve ({split_name})")
...
def plot_metrics_summary(split_name: str, data: dict) -> None:
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title(f"YOLOv5 Metrics Summary ({split_name})")
