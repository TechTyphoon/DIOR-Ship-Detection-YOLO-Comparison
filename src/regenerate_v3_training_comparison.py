...
def plot_training_comparison():
    ...
    ax.set_ylim(0.0, 1.0)
    ax.set_ylabel("Score")
    ax.set_title("YOLOv3 Training Metrics Comparison Across Splits")
    ax.legend()
    ax.grid(axis="y", alpha=0.3)

    out_dir = BASE_DIR / "runs" / "detect" / "v3_metrics" / "comparisons"
    out_dir.mkdir(parents=True, exist_ok=True)
    fig.tight_layout()
    fig.savefig(out_dir / "v3_splits_training_metrics_comparison.png", dpi=200)
    plt.close(fig)
